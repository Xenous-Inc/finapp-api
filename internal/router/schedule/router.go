...
func (r *Router) filterMiniSchedule(scheduleResponse []models.Schedule) []dto.MiniScheduleItem {
	...
	for i := 1; i < len(scheduleResponse); i++ {
		...
		if scheduleResponse[i].LessonNumberStart != scheduleResponse[i-1].LessonNumberStart || scheduleResponse[i].Date != scheduleResponse[i-1].Date || scheduleResponse[i].LessonNumberEnd != scheduleResponse[i-1].LessonNumberEnd {
